...
scr_netwarn_highping::
    Optional absolute high ping warning threshold, in milliseconds. Default
    value is 0, which disables absolute high ping warnings. This keeps stable
    high-latency servers from constantly drawing warnings.

scr_netwarn_ping_adaptive::
    Enables adaptive ping spike and jitter detection relative to the current
    rolling ping baseline. Default value is 1.

scr_netwarn_spike_ms, scr_netwarn_spike_pct, scr_netwarn_jitter_ms::
    ...
    baseline when adaptive detection is enabled. Default values are 60, 25 and
    18.

scr_netwarn_stall_ms, scr_netwarn_stall_crit_ms::
    Time since last received server packet, in milliseconds, before drawing a
    connection stall warning and critical stall warning. Default values are 500
    and 1000.

scr_netwarn_pred_warn, scr_netwarn_pred_crit::
    Client prediction correction thresholds. Default values are 24 and 80.
...
vid_fullscreen_multimonitor::
    On Windows, enables modern multi-monitor behavior in fullscreen mode: the
    mouse is released while the in-game console is open, and the window remains
    visible when focus moves to another screen. Default value is 1.

vid_multimonitor::
    On Windows, configures borderless window and focus settings for multi-display
    setups. Enabling this automatically configures both `win_noborder` and
    `vid_fullscreen_multimonitor` accordingly. Default value is 0.
