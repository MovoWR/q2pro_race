...
scr_netbar::
    Toggles drawing of a subtle full-width network history bar. Default value is
    1.
      - 0 -- do not draw the bar
      - 1 -- draw a thin event-colored history bar
      - 2 -- draw a hybrid ping-height/event bar
      - 3 -- draw debug-style hybrid bar

scr_netbar_y::
    Vertical placement of the network bar. Negative values align from the
    bottom edge of the screen. Default value is -1.

scr_netbar_h::
    Height of the network bar in pixels. Default value is 4.

scr_netbar_alpha::
    Opacity of normal network samples. Default value is 0.20.

scr_netbar_bad_alpha::
    Opacity of warning and critical network samples. Default value is 0.75.

scr_netbar_history_ms::
    Time window represented by the network bar, in milliseconds. Default value
    is 60000.

scr_netbar_ping_mode::
    Controls whether ping affects sample height. Default value is 0.
      - 0 -- draw fixed-height event samples
      - 1 -- draw ping-height samples
      - 2 -- draw hybrid samples with quiet normal traffic

scr_netbar_labels::
    Toggles a small label for the strongest recent network warning. Default
    value is 1. Values higher than 1 also draw the last measured ping.

scr_netbar_notice::
    Toggles a larger short incident notice for network events that may affect a
    run. Default value is 2.
      - 0 -- do not draw notices
      - 1 -- draw notice near the network bar
      - 2 -- draw notice at top center

scr_netbar_notice_y::
    Top-center notice vertical position. Default value is 48.

scr_netbar_notice_ms::
    Visibility time for recent incident notices, in milliseconds. Default value
    is 1200.

scr_netbar_notice_alpha::
    Opacity of incident notice text. Default value is 0.85.

scr_netbar_notice_stall, scr_netbar_notice_loss, scr_netbar_notice_pred,
scr_netbar_notice_choke, scr_netbar_notice_frame, scr_netbar_notice_jitter,
scr_netbar_notice_spike, scr_netbar_notice_ping::
    Toggles each incident notice type independently. Default value is 1 for
    every notice type. These only affect the large incident notice; the network
    bar history and small bar label keep using all detected incidents.

sh_netwarn_ping_adaptive::
    Enables adaptive ping spike and jitter detection relative to the current
    rolling ping baseline. Default value is 1.

sh_netwarn_spike_ms, sh_netwarn_spike_pct, sh_netwarn_jitter_ms::
    Thresholds for ping spike and jitter warnings. Spike detection uses the
    larger of the millisecond threshold and the percentage of the rolling ping
    baseline when adaptive detection is enabled. Default values are 80, 50 and
    40.
...
configuration. Back discards unapplied changes. See
...
cl_menu_cursor::
    Specifies the texture/image to use for the in-game menu cursor. Valid values
    are `none` (no cursor), `cross` (or `ch1`), `dot` (or `ch2`), `angle` (or `ch3`),
    `ch4`, `ch5`. Default value is `ch1`.
