...
with existing Quake 2 ports and licensed under GPLv2 or later. This document provides
...
This copy describes q2pro_race and its registered defaults. Saved configs and
command-line settings can override them; optional features depend on the build.
See link:custom-cvars.md[the custom settings reference] for jump overlays,
FPS bindings, bind reminders, menu customization, and the HUD editor.

JumpMod Keyboard Shortcuts
-------------------------

During active play with `game jump`, hold Ctrl and press one of these keys:

[cols="1,1,2",options="header"]
|=================
| Shortcut | Command | Action
| Ctrl+M | `inven` | Open or close the JumpMod menu
| Ctrl+Up | `invprev` | Select the previous menu item
| Ctrl+Down | `invnext` | Select the next menu item
| Ctrl+Enter | `invuse` | Activate the selected menu item
|=================

These shortcuts are built into the client and need no config bindings. Each
press sends one command; holding the key does not repeat it. The server decides
what these commands do when its menu is closed.

They apply only to game input, not chat, the console, client menus, HUD editing,
or demo playback. Adding Alt or Shift disables the shortcut. Ordinary bindings
for M, the arrows, and Enter still work without Ctrl; Ctrl's own binding remains
active. During a shortcut, the other key's normal binding is suppressed until
that key is released, even if Ctrl is released first.
...
r_maxfps::
    Specifies the rendering rate limit when ‘cl_async’ is positive. Default
    value is 120. Nonzero values are clamped to 10-1000; 0 selects the 1000 FPS
    ceiling. The scheduler rounds the frame interval to whole milliseconds.

cl_maxfps::
    Specifies the physics rate limit when ‘cl_async’ is positive. Otherwise,
    limits both rendering and physics. Default value is 120. Nonzero values
    are clamped to 10-125; 0 selects the 125 FPS ceiling. Whole-millisecond
    rounding means the actual interval may differ from the requested rate.
...
cl_gun::
    ...
    protocol, disabling this saves some bandwidth since the server stops
    sending gun updates at all. Default value is 0 (hidden).
      - 0 — don't draw gun
...
cl_warn_on_fps_rounding::
    ...
    due to rounding. Default value is 0 (disabled).
...
cl_railcore_width::
    Width of the rail core beam. Default value is 2.
...
These cvars and the ‘ogg’ command require FFmpeg support in the build
(`avcodec`). The configured Windows CI build disables that feature.
...
con_history::
    Specifies how many lines to save into `.conhistory` in the active game
    directory, alongside `q2config.cfg` on Windows or `config.cfg` elsewhere.
    History is saved before exiting Q2PRO
    and before restarting the filesystem. Changing game directories loads that
    game's history; restarting the same directory does not reload history.
    If a game has no history file yet, the legacy `baseq2/.conhistory` is read
    as a starting point. Subsequent saves use the active game directory.
    History uses `homedir` when configured, otherwise `basedir`. Maximum number of
    history lines is 128. Default value is 128; 0 disables loading and saving.
...
scr_showturtle::
    ...
    corner of the screen. Default value is 2 (draw all errors). Value 1 omits
    SUPPRESSED, CLIENTDROP and SERVERDROP; 0 disables the indicators.
...
scr_scale::
    Scaling factor of the HUD elements. Default value is 2. Set to 0 to scale
    automatically for the display resolution, or 1 to disable scaling.
...
scr_font::
    Font used for drawing HUD text. Default value is "prochars".
...
sh_lagometer_x::
sh_lagometer_y::
    Position of the lagometer and the legacy ‘scr_lag_draw’ graph/connection
    icon in HUD units. Negative values align from the opposite screen edge.
    Default values are 166 and -1.

sh_netmeter::
    Selects the jump network display: 0 off, 1 lagometer, 2 full-width netgraph,
    or 3 histogram. Default value is 3.

sh_netmeter_min_ms::
sh_netmeter_max_ms::
    Minimum and maximum latency on the fixed graph scale. Default values are
    0 and 150 milliseconds. Also used by the legacy ping graph.

sh_netmeter_adaptive::
    Adapts the graph scale to recent latency. Default value is 1. Set to 0 to
    use the fixed minimum/maximum scale.

sh_netalert::
    Enables network incident text, independently of ‘sh_netmeter’. Default
    value is 1. ‘PACKET LOSS’ reports server-to-client loss; ‘CLIENT DROP’
    reports missing client movement commands, including server prediction
    after backup commands run out. Other alerts are ‘PING SPIKE’ and
    ‘NETWORK JITTER’.

sh_netwarn_ping_adaptive::
    Enables adaptive spike and jitter detection from recent clean samples.
    Default value is 1. Detection warms up over 32 clean samples.

sh_netwarn_spike_ms, sh_netwarn_spike_pct, sh_netwarn_jitter_ms::
    Spike and jitter thresholds. Default values are 300 milliseconds,
    100 percent and 100 milliseconds. Adaptive spike detection uses the larger
    of the millisecond threshold (with a 20 ms floor) and the percentage of
    the rolling ping baseline. Adaptive jitter detection uses the larger of
    its threshold (with a 10 ms floor) and baseline jitter plus a quarter of
    the spike threshold. With adaptation disabled, the millisecond values
    are used directly.

See link:custom-cvars.md#network-meter-core[the network settings reference] for
bar dimensions, history, colors, alert positions and individual alert switches.
The former ‘scr_lag_x/y/min/max’ and ‘scr_netbar_*’ settings are not registered
by this client; use the current ‘sh_*’ controls above.
...
scr_chathud_time::
    ...
    value is 30. Set to 0 to keep lines from fading out.
...
scr_chathud_y::
    ...
    edge of the screen instead of the top edge. Default value is -150.
...
On Windows, the Display menu provides Windowed, Borderless fullscreen and
Exclusive fullscreen modes, a monitor selector, and mode-specific size/rate
controls. Apply starts a 15-second Keep/Revert confirmation before saving the
archived settings. VSync-only changes commit immediately. Back discards
unapplied changes. See
link:display-settings.md[Display settings] for behavior and validation details.
...
vid_modelist::
    ...
    Confirming an exclusive mode in the Windows Display menu saves its entry
    in this list and archives the list together with the selected index.
...
vid_fullscreen::
    ...
    On Windows, `vid_fullscreen` with `vid_noborder` set selects borderless
    fullscreen on the selected monitor instead of exclusive fullscreen.

vid_monitor::
    On Windows, selects a display by its Windows device identifier. Empty means
    automatic selection. The Display menu populates this archived value.

_vid_fullscreen_borderless::
    Remembers the last confirmed fullscreen type for Alt-Enter on Windows.
    Empty preserves the initial exclusive-fullscreen preference.
...
vid_noborder::
    On Windows, hides the main window border. If combined with non-zero
    `vid_fullscreen`, the Windows port uses borderless fullscreen instead of
    changing the desktop display mode. Default value is 0. Selecting Windowed
    in the Display menu clears this, `win_notitle`, and `win_noresize` to restore
...
win_menu_cursor::
    Specifies the Windows system cursor style to display when the in-game menu is
    active. Valid values are `arrow`, `cross`, `hand`, `ibeam`, `wait`, `no`,
    `size`. Values `1`, `on`, and `true` are aliases for `arrow`; values `0`,
    `off`, and `none` disable the system cursor override. Default value is
    `arrow`.
...
gl_beamstyle::
    Specifies drawing style of laser beams. Default value is 0.
      ...
      - 2 — billboard-type beam with proximity heatmap (color changes with distance)
      - 3 — lead-arrow head style (tapered towards the direction of travel)
      - 4 — combined proximity heatmap and lead-arrow head style
...
gl_lava_intensity::
    Specifies a multiplier applied to non-transparent lava textures in
    addition to the global ‘intensity’ value when using the shader rendering
    backend. The allowed range is 0.1-5. Default value is 1 (identity).
...
gl_profile::
    Specifies custom profile and version of OpenGL context to create. The build
    default is "gl3.2" on macOS, "es1" with `opengl-es1`, and empty otherwise.
    An empty string requests the highest version compatibility profile context.
    Can be set to "gl" to create OpenGL core profile context,
...
gl_screenshot_format::
    Specifies image format ‘screenshot’ uses. Available formats depend on the
    build: "png", "jpg" and "tga" require their respective format support.
    The cvar defaults to "jpg" with JPEG support, otherwise "png" with PNG
    support. A TGA-only build writes TGA without registering this cvar.
...
r_texture_formats::
    ...
    The build includes enabled formats in the order "png jpg tga"; formats
    whose support was disabled are omitted from the default.
...
These cvars are available when built with libcurl support.
...
cl_menu_cursor::
    Specifies the image for the in-game menu cursor. Named choices include
    `none`, `cross` (`ch1`), `dot` (`ch2`), `angle` (`ch3`), `ch4` and `ch5`;
    other image names are also accepted. Default value is `ch1`. Windows uses
    its system cursor outside exclusive fullscreen. A missing image falls back
    to a drawn cross; `none` hides it except during display confirmation.

cl_input_keys::
    Read-only cvar that tracks and displays the currently pressed movement and
    jump/crouch keys as a string (e.g. `[W][A][S][D] [J][C]`). Empty when not in an
    active game or spectating.
...
ui_menu_bar_image::
    Image used for the menu background bars. Default value is
    "q2jump_background". See link:custom-cvars.md#ui-and-menus[menu settings]
    for image layout, opacity, colors and the player-model backdrop.
...
cl_fps:: physics rate implied by the current whole-millisecond scheduler interval;
         empty when that separate interval is inactive
cl_mfps:: measured main client-loop frame rate
cl_mmps:: measured movement-command generation rate in movements per second
cl_pps:: measured movement-packet transmission rate in packets per second
cl_ups:: horizontal player speed in world units per second
cl_rups:: 3D player speed, including vertical velocity, in world units per second
r_fps:: rendering rate implied by the current whole-millisecond scheduler interval;
        empty when that separate interval is inactive
r_mfps:: measured rendering frame rate
...
.Demo packet sizes
******************
Without protocol extensions, a demo recorded from Q2PRO or R1Q2 protocol is
...
per demo by ‘record’ command options. When the server uses extended configstrings,
the recorder instead writes the extended demo protocol with a 32768-byte message
buffer, regardless of the standard/extended packet-size option. Such recordings
require a client that supports that extended demo format.
******************
...
toggle <cvar> [value1 value2 ...]::
    ...
    If explicit _values_ are specified, cycle through them. If the current
    value is outside the list, select its first value. For example,
    `toggle cl_maxfps 30 120` selects 30 when `cl_maxfps` is 60.
...
draw <name> <x> <y> [color]::
    ...
    in optional _color_. Use `dynamic` as the color for supported speed macros,
    for example `draw cl_ups -1 80 dynamic`. There is no separate `draw_dynamic`
    command. Calling `draw` without arguments lists registered objects.
...
===================
-------------------
/draw r_mfps -1 -1  // measured render FPS, bottom right
...
Each format-specific command requires that image format to be enabled in the
build. The generic `screenshot` command chooses from available formats.
...
passive::
    ...
    servers behind NATs or firewalls. See
    link:server.asciidoc[the ‘pickclient’ server command] for more details.
...
remotemode <address> <password>::
    ...
    to the remote server. Press Ctrl+D or close console to exit this mode.
