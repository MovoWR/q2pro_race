...
NOTE: This is a reference for the retained Q2PRO server source. In this fork,
`meson.build` comments out the standalone `q2proded` target and builds the
`q2pro_race` client and game library. The current build does not produce a
server executable; this manual does not imply that these server commands are
available in the client. Optional server features also depend on their build
configuration. See link:../INSTALL.md[INSTALL.md] for the current build scope.
...
sv_allow_map::
    Controls the ‘map’ command behavior. ‘map’ is often mistakenly used by
...
These variables control files served through legacy server UDP downloads.
They do not configure a separate HTTP content server or define the client
inbound file policy. The client applies its own path and extension checks;
see link:client.asciidoc[the client manual].
...
allow_download::
    ...
    is 0 for a dedicated server and 1 otherwise.
...
allow_download_others::
    Allows the server to serve files from any subdirectory other than those
    listed above. Default value is 0. This is not a client-side control for
    receiving arbitrary file types or archive members.
...
sv_mvd_begincmd::
    ...
    MVD observer has its own command buffer and each ‘wait’ cycle lasts 100 ms
...
com_debug_break::
    ...
    value is 0 (disabled). Available only in debug builds.
...
pickclient <address:port>::
    ...
    specified _port_ must be reachable. See the ‘passive’ command in
    link:client.asciidoc[the client manual] for more details.
